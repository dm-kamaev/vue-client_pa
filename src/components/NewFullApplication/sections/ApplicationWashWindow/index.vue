<template>
  <div class="application-wash-window">
    <application-header @backwards="orderPrevStep">
      Добавьте к мытью окон
    </application-header>

    <calculator-item v-if="length" class="application-wash-window__section">
      <swiper class="carousel" :options="options">
        <swiper-slide  v-for="slide in config.additionWorks" :key="slide" class="application-wash-window__carousel-item carousel__item">
          <service-card :serviceId="slide" v-model="controllers.cards[slide].value.value" @input="change"/>
        </swiper-slide>
      </swiper>
    </calculator-item>

    <div class="application-wash-window__btns-wrapper">
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        defaultControllers: state => state.order.window.controllers
      }),
      ...
      config() {
        return this.$store.state.services.objectServices[this.serviceId]
      },
      ...
      listServicesConfig() {
        ...
        this.config.additionWorks.forEach((item) => {
          services.push(this.$store.state.services.objectClasses[item])
        })
    ...
    methods: {
      ...
      ...mapActions(['getOrderPrice', 'setWindow', 'orderNextStep', 'orderPrevStep', 'sendContacts']),
    ...
    mounted() {
      ...
      analytic.sendEvent({name: "form", value: "order_new_window", ga_action: "open"}, ['ga', 'af', 'fb'])
    }
  }
</script>
